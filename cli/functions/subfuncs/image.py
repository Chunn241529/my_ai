...
def scan_directory(directory):
    ...
    if not os.path.exists(directory):
        ...
        return configs
    for file_path in glob.glob(os.path.join(directory, "*.safetensors")) + glob.glob(
...
if not os.path.exists(model_path):
    ...
    raise FileNotFoundError(
        "Không tìm thấy tệp mô hình .safetensors trong thư mục model"
    )
...
def encode_prompt_embeddings(pipe, prompt, negative_prompt, chunk_size=77):
    try:
        ...
        # Mã hóa negative prompt
        tokenized_neg_output = pipe.tokenizer(
            negative_prompt, return_tensors="pt", truncation=False, padding=False
        )
        tokenized_negative = tokenized_neg_output.input_ids.to("cuda")
        negative_length = tokenized_negative.shape[-1]

        # Xác định độ dài tối đa dựa trên cả prompt và negative prompt
        max_length = max(prompt_length, negative_length, chunk_size)

        # Đệm hoặc cắt cả hai để có cùng độ dài
        tokenized_output = pipe.tokenizer(
            prompt,
            return_tensors="pt",
            truncation=True,
        ...
        if prompt_embeds.shape[1] != negative_prompt_embeds.shape[1]:
            logging.error(
                f"Kích thước embedding không khớp: prompt_embeds {prompt_embeds.shape} != "
                f"negative_prompt_embeds {negative_prompt_embeds.shape}"
            )
            return None, None

        logging.info(
            f"Đã mã hóa prompt và negative prompt với độ dài {max_length} token"
        )
    ...
    for emb_config in embedding_configs:
        ...
        if emb_token in tokens:
            ...
            try:
                ...
                logging.error(
                    f"Lỗi khi tải embedding {emb_token} từ {emb_path}: {str(e)}"
                )
                failed_embeddings.append(emb_token)
...
def load_loras(pipe, prompt, final_prompt, lora_configs, default_lora_scale):
    ...
    lora_matches = re.findall(
        r"\<lora:([^:]+):(\d*\.\d+)\>", f"{prompt} {final_prompt}"
    )
    lora_dict = {token: float(scale) for token, scale in lora_matches}

    # Xóa tất cả adapter hiện có để tránh xung đột
    try:
        pipe.disable_lora()
        logging.info("Đã xóa tất cả adapter LoRA hiện có")
    except Exception as e:
        logging.warning(f"Không thể xóa adapter LoRA hiện có: {str(e)}")
    ...
    for i, lora_config in enumerate(lora_configs):
        ...
        if lora_token in lora_dict or lora_token in final_prompt.lower():
            try:
                if os.path.getsize(lora_path) > 0:
                    # Tải LoRA với tên adapter duy nhất
                    pipe.load_lora_weights(lora_path, adapter_name=lora_id)
                    scale = lora_dict.get(lora_token, default_lora_scale)
                    active_lora_ids.append(lora_id)
                    active_lora_scales.append(scale)
                    loaded_loras.append(f"{lora_token} (scale: {scale})")
                    logging.info(
                        f"Đã tải LoRA {lora_token} với adapter {lora_id} và scale {scale}"
                    )
                else:
    ...
    if active_lora_ids:
        try:
            ...
            logging.info(
                f"Đã thiết lập adapters: {active_lora_ids} với trọng số {active_lora_scales}"
            )
        except Exception as e:
            logging.error(f"Lỗi khi thiết lập adapters: {str(e)}")
            # Nếu có lỗi, vô hiệu hóa LoRA và tiếp tục
            pipe.disable_lora()
            active_lora_ids = []
            active_lora_scales = []
...
def genImage(
    ...
    try:
        ...
        with redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
            ...
            logging.info(
                f"Khởi tạo pipeline thành công trong {time.time() - start_time:.2f}s"
            )
        ...
        try:
            ...
            logging.info(
                ...
                + (
                    " với Karras sampling"
                    if scheduler_kwargs.get("use_karras_sigmas")
                    else ""
                )
            )
        except Exception as e:
        ...
        base_negative_prompt = (
            "ng_deepnegative_v1_75t,badhandv4,hands,(bad anatomy:1.8),(extra limbs:2.0),(extra arms:2.0),(extra hands:2.0),"
            "(extra fingers:2.0),(missing fingers:1.8),(fused fingers:1.8),(deformed hands:1.8),(deformed fingers:1.8),"
            "(extra legs:2.0),(extra feet:2.0),(extra toes:2.0),(missing toes:1.8),(deformed feet:1.8),(deformed toes:1.8),"
            "(distorted proportions:1.5),(unrealistic anatomy:1.5),(low quality:1.5),(blurry:1.5),(poorly drawn hands:1.8),"
            "(poorly drawn feet:1.8),(asymmetrical limbs:1.5),(unnatural joints:1.5)"
        )
        if negative_prompt:
        ...
        if (
            "girl" in cleaned_prompt.lower() or "woman" in cleaned_prompt.lower() or "boy" in cleaned_prompt.lower() or "man" in cleaned_prompt.lower()
        ) and not (
            "girl" in cleaned_prompt.lower() and "nude" in cleaned_prompt.lower()
        ):
            final_prompt = f"<lora:ClothingAdjuster3:0.1>,<lora:instantphoto:0.8>,<lora:hand_v2:1.5>,best quality,masterpiece,highly detailed,ultra highres,photorealistic,ultra-detailed,high-resolution,cinematic lighting,highly detailed face,beautiful face,Detailed hands,Perfect hands,Detailed fingers,Perfect fingers,<lora:Detail:1.0>,{cleaned_prompt},<lora:better_body:0.8>,"
        elif "girl" in cleaned_prompt.lower() and "nude" in cleaned_prompt.lower():
            final_prompt = f"<lora:ClothingAdjuster3:1.0>,<lora:instantphoto:1.0>,<lora:hand_v2:1.5>,best quality,masterpiece,highly detailed,ultra highres,photorealistic,ultra-detailed,high-resolution,cinematic lighting,highly detailed face,beautiful face,Detailed hands,Perfect hands,Detailed fingers,Perfect fingers,<lora:Detail:1.0>,({cleaned_prompt}),<lora:Girlfriend:1.0>,"
        else:
            final_prompt = f"best quality,masterpiece,highly detailed,ultra highres,photorealistic,ultra-detailed,high-resolution,cinematic lighting,highly detailed,{cleaned_prompt}"
        ...
        if prompt_embeds is None or negative_prompt_embeds is None:
            logging.error("Không thể tạo embedding cho prompt hoặc negative prompt")
            raise RuntimeError(
                "Không thể tạo embedding cho prompt hoặc negative prompt"
            )

        # Kiểm tra bộ nhớ GPU trước khi tạo ảnh
        free_memory = (
            ...
            if hires_fix:
                ...
                for i, img in enumerate(images):
                    hires_kwargs = {
                        ...
                        "strength": 0.3,
                        "target_size": (
                            int(width * hires_scale),
                            int(height * hires_scale),
                        ),
                    }
