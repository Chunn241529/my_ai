...
def reason_with_ollama(query, context, model=model_reason):
    ...
    prompt = f"""
        ...
        - Cuối cùng, trả lời logic, rõ ràng, đúng trọng tâm câu hỏi, dựa hoàn toàn trên lập luận, sử dụng markdown.
        Viết tự nhiên, mạch lạc, chỉ dùng thông tin từ '{context}', không thêm dữ liệu ngoài.
        Không có phần mở đầu như "To answer this: ", tương tự với những phần khác.
    """
...
def summarize_answers(query, all_answers, model=model_curent):
    ...
    summary_prompt = f"""
        ...
        Trả về một đoạn văn dài, cực kỳ chi tiết, bao quát mọi khía cạnh hợp lệ từ thông tin thu thập, không bỏ sót bất kỳ ý nào có giá trị. Toàn bộ nội dung phải được định dạng bằng cú pháp Markdown.
    """
