...
import base64
from web.functions.repository.message import MessageRepository
...
def generate_image(query, model=model_gemma):
    """Tạo prompt cho Stable Diffusion bằng tiếng Anh dựa trên câu hỏi người dùng."""
    prompt = f"""
        Cải tiến prompt Stable Diffusion trong "{query}" ngắn gọn bằng tiếng Anh (English) < 77 token\n
        Chỉ trả về 1 prompt duy nhất, không giải thích, không thêm nội dung khác, không dùng emoji
    """
    return query_ollama(prompt, model, num_predict=200, temperature=0.1)


def image_chat_(query, image_path="", url_image="", model=model_qwen):
    prompt = ""
    if image_path and os.path.exists(image_path):
        ...
        prompt = f"""
            Dùng `tiếng Việt` để trả lời.
            Phân tích câu hỏi: '{query}' một cách tự nhiên, thân thiện và mạch lạc.
            Điều chỉnh giọng điệu phù hợp với ý định của câu hỏi, nhưng không đề cập đến phân tích hay cảm xúc trong câu trả lời.
            Không nhắc lại câu hỏi.
            Đưa ra 1 prompt stable diffusion ngắn gọn bằng tiếng Anh (English). không cần ghi lại tiêu đề.
            Sau đó, hãy tạo hình ảnh dựa trên prompt đã tạo.
            Ví dụ: `Được rồi, tôi sẽ tạo hình ảnh dựa trên những prompt sau đây`
        """
    return query_ollama(
        prompt,
        model,
